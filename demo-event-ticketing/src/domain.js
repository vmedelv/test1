...
const { db, persist, id, ticketCode, slugify } = require('./store');
...
function checkout(slug, input) {
  ...
  // Reserva de stock (dentro del mismo tick de JS, sin await de por medio:
  // en Node de un solo hilo esto evita el oversell sin necesitar un lock
  // externo como Redis, suficiente para esta demo).
...
// Simula el "commit" que en produccion llega desde Transbank Webpay Plus
// (redirect + confirmacion sincrona) o el webhook de Flow (server-to-server).
