'use strict';
/* Siembra datos de ejemplo la primera vez que se abre el demo en este
 * navegador (o al pulsar "Reiniciar demo"). Espejo de
 * demo-remindaclinic/src/seed.js. */
(function () {
...
const PROFESSIONALS = [
  { name: 'Dra. Carolina Rojas', specialty: 'Medicina General', color: '#2563eb', slotMinutes: 30, days: [1, 2, 3, 4, 5], blocks: [['09:00', '13:00'], ['15:00', '18:00']] },
  { name: 'Dr. Matias Fuentes', specialty: 'Nutricion', color: '#16a34a', slotMinutes: 45, days: [1, 3, 5], blocks: [['10:00', '13:00']] },
  { name: 'Dra. Valentina Soto', specialty: 'Dermatologia', color: '#db2777', slotMinutes: 20, days: [2, 4, 6], blocks: [['09:00', '12:00'], ['16:00', '19:00']] },
];
  ...
  for (const prof of profs) {
    ...
    for (let offset = 0; offset <= 7 && booked < targetPerProf; offset++) {
      ...
      for (let i = 0; i < free.length && booked < targetPerProf; i += 2) {
        ...
        patientIx += 1;
        try {
          const appt = Domain.createAppointment({ professionalId: prof.id, date, time: free[i].time, ...patient });
          created += 1; booked += 1;
          if (created % 3 === 0) Domain.confirmAppointment(appt.id);
        } catch (e) { /* hueco tomado: seguimos */ }
      }
    }
  }
  return created;
...
// Siembra solo si la base esta vacia.
function ensureSeeded() {
  if (!Store.db.professionals.length) run();
}

window.Seed = { run, ensureSeeded };
